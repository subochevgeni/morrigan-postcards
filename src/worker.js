...
function formatSetAccessWordPrompt(state, ttlDays) {
  return (
    '✏️ Set access phrase\n' +
    'Send command in this format:\n' +
    '/setaccessword your-new-secret\n\n' +
    'If you want an auto-generated phrase, use:\n' +
    '/rotateaccess\n\n' +
    formatAccessStateInfo(state, ttlDays)
  );
}

async function applyAccessPhraseUpdate(env, { chatId, candidateRaw = '', autoGenerateIfEmpty = false, via = 'rotateaccess' }) {
  let nextPhrase = String(candidateRaw || '').trim();
  ...
  if (!nextPhrase) {
    if (!autoGenerateIfEmpty) {
      return {
        ok: false,
        missingInput: true,
        state: await getEffectiveAccessState(env),
      };
    }
  ...
  if (!parsedPhrase.ok) return { ok: false, error: parsedPhrase.error };
  ...
  return {
    ok: true,
    autoGenerated,
    updatedState: await getEffectiveAccessState(env),
  };
}
...
function adminKeyboard() {
  return {
    keyboard: [
      ...
      [{ text: '🔐 Access Word' }, { text: '✏️ Set Access Word' }],
...
function adminHelpText() {
  ...
  return (
    ...
    '/setaccessword <new_phrase> — set exact phrase from Telegram\n' +
...
function normalizeAdminText(text) {
  ...
  if (t === '✏️ Set Access Word') return '/setaccessword';
...
async function handleTelegram(request, env) {
  ...
  if (isAdmin && msgText.startsWith('/')) {
    ...
    if (cmd === '/setaccessword' || cmd === '/rotateaccess') {
      const candidateRaw = parts.slice(1).join(' ').trim();
      const isExplicitSet = cmd === '/setaccessword';
      const result = await applyAccessPhraseUpdate(env, {
        chatId,
        candidateRaw,
        autoGenerateIfEmpty: !isExplicitSet,
        via: isExplicitSet ? 'setaccessword' : 'rotateaccess',
      });

      if (!result.ok && result.missingInput) {
        await tgSend(
          env,
          chatId,
          formatSetAccessWordPrompt(result.state, getAccessTtlDays(env)),
          adminKeyboard()
        );
        return json({ ok: true });
      }
      if (!result.ok && result.error) {
        await tgSend(
          ...
          chatId,
          'Usage: ' +
            (isExplicitSet ? '/setaccessword <new_phrase>\n' : '/rotateaccess [new_phrase]\n') +
            result.error,
          adminKeyboard()
        );
        return json({ ok: true });
      ...
      if (!result.ok && result.alreadyActive) {
        await tgSend(env, chatId, 'This phrase is already active.', adminKeyboard());
        return json({ ok: true });
      }
      ...
      await tgSend(
        env,
        chatId,
        '✅ Access phrase rotated (' +
          (result.autoGenerated ? 'auto-generated' : 'custom') +
          ').\n\n' +
          formatAccessStateInfo(result.updatedState, getAccessTtlDays(env)),
        adminKeyboard()
      );
      return json({ ok: true });
    }
